...
class SessionRedisTest extends TestCase
{
    public function testSessions()
    {
        ...
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=0&locking=false&random=' . $random_hex);
        ...
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=1&locking=false');
        ...
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=2&locking=false');

        //does get work
        $this->assertTrue($random_hex === $r->getBody()->getContents(), 'Session storage did not save or get.');

        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=3&locking=false');
        ...
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=6&locking=false');
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=7&locking=false');

        $this->assertEquals('1', $r->getBody()->getContents(), 'Remember me was not set to 1 in DB.');

        //does now setting remember me to 0 work
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=8&locking=false');
        $r = $client->request('GET', 'http://127.0.0.1:8080/SessionRedis.php?tests=7&locking=false');
